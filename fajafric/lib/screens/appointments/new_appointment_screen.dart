...
/// Affiche le formulaire de nouveau RDV en bottom sheet (reste dans la sidebar).
class NewAppointmentScreen extends StatefulWidget {
  final Map<String, dynamic>? medecin;
  final VoidCallback? onCreated;

  const NewAppointmentScreen({super.key, this.medecin, this.onCreated});

  /// Ouvre le formulaire en modal bottom sheet.
  static Future<bool> show(BuildContext context,
      {Map<String, dynamic>? medecin, VoidCallback? onCreated}) async {
    final result = await showModalBottomSheet<bool>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => NewAppointmentScreen(medecin: medecin, onCreated: onCreated),
    );
    return result == true;
  }
    ...
    return DraggableScrollableSheet(
      ...
      builder: (_, ctrl) => Container(
        ...
        child: Column(children: [
          ...
          const SizedBox(height: 16),
          Expanded(
            child: ListView(controller: ctrl, padding: const EdgeInsets.fromLTRB(24, 0, 24, 32), children: [

              // ── MÉDECIN ──────────────────────────────────────────────
              const Text('Médecin', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700)),
                ...
                Container(
                  ...
                  child: Row(children: [
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(medecinNom!, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15)),
                      Text(_selectedMedecin!['specialite'] ?? 'Médecin généraliste',
                          style: const TextStyle(fontSize: 12.5, color: AppTheme.teal)),
                    ])),
                ...
                if (_filteredDoctors.isEmpty)
                  Padding(
                    ...
                    child: Center(child: Text(
                      _doctors.isEmpty ? 'Aucun médecin disponible' : 'Aucun résultat pour "$_searchQuery"',
              ...
              if (_error != null) ...[
                Container(
                  ...
                  decoration: BoxDecoration(color: AppTheme.red.withOpacity(0.08), borderRadius: BorderRadius.circular(10)),
              ...
              // ── TYPE ──────────────────────────────────────────────────
              const Text('Type de consultation', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700)),
                  ...
                  return GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(color: sel ? AppTheme.teal : AppTheme.line, width: sel ? 2 : 1),
                      ),
                      child: Row(mainAxisSize: MainAxisSize.min, children: [
                        Icon(t['icon'] as IconData, size: 16, color: sel ? AppTheme.teal : AppTheme.inkSoft),
              ...
              // ── DATE ──────────────────────────────────────────────────
              const Text('Date', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700)),
              const SizedBox(height: 8),
              GestureDetector(
                onTap: () async {
                  final d = await showDatePicker(
                    ...
                    builder: (ctx, child) => Theme(
                      data: ThemeData.light().copyWith(colorScheme: const ColorScheme.light(primary: AppTheme.teal)),
                      child: child!,
                    ),
                  );
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: AppTheme.bgCard,
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(color: _selectedDate != null ? AppTheme.teal : AppTheme.line),
                  ),
                  child: Row(children: [
                    ...
                    Text(
                      _selectedDate != null
                          ...
                          : 'Choisir une date',
                      style: TextStyle(fontSize: 14, color: _selectedDate != null ? AppTheme.ink : AppTheme.inkSoft),
                    ),
                  ]),
                ),
              ),
              ...
              // ── HEURE ─────────────────────────────────────────────────
              const Text('Heure', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700)),
              const SizedBox(height: 8),
              GestureDetector(
                onTap: () async {
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: AppTheme.bgCard,
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(color: _selectedTime != null ? AppTheme.teal : AppTheme.line),
                  ),
                  child: Row(children: [
                    ...
                    const SizedBox(width: 10),
                    Text(
                      _selectedTime != null ? _selectedTime!.format(context) : 'Choisir une heure',
                      style: TextStyle(fontSize: 14, color: _selectedTime != null ? AppTheme.ink : AppTheme.inkSoft),
                    ),
                  ]),
                ),
              ),
              ...
              // ── MOTIF ─────────────────────────────────────────────────
              const Text('Motif (optionnel)', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700)),
              const SizedBox(height: 8),
              TextField(
                controller: _motifCtrl,
                maxLines: 3,
                decoration: const InputDecoration(hintText: 'Décrivez votre motif de consultation…'),
              ),
